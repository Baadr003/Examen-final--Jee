...
public ResponseEntity<?> updateUserProfile(
    ...
    try {
        ...
        return ResponseEntity.ok(new AuthResponseDTO("Profil mis à jour avec succès", true, updatedUser.getId()));
    } catch (Exception e) {
        return ResponseEntity.badRequest()
                .body(new AuthResponseDTO("Erreur: " + e.getMessage(), false, null));
    }
...
public ResponseEntity<?> requestPasswordReset(@RequestParam String email) {
    try {
        ...
        return ResponseEntity.ok()
            .body(new AuthResponseDTO("Code de réinitialisation envoyé", true, null));
    } catch (Exception e) {
        log.error("Password reset request failed: {}", e.getMessage());
        return ResponseEntity.badRequest()
            .body(new AuthResponseDTO(e.getMessage(), false, null));
    }
}
...
public ResponseEntity<?> resetPassword(@RequestBody PasswordResetDTO resetDTO) {
    try {
        ...
        return ResponseEntity.ok(new AuthResponseDTO("Mot de passe mis à jour avec succès", true, null));
    } catch (Exception e) {
        return ResponseEntity.badRequest()
            .body(new AuthResponseDTO(e.getMessage(), false, null));
    }
